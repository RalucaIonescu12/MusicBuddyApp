...
//                    Map<String, Map<String, Double>> audioFeatures = new HashMap<>(); //id:{"danceability":0.3,...}}
    ...
    public boolean checkIfTrackAdded(TrackSearchItem selectedItem)
    {
        if(recommendationFilters.get("seed_tracks")!=null && recommendationFilters.get("seed_tracks").containsKey(selectedItem.getSongName()))
            return true;
        return false;
    }
    public boolean checkIfArtistAdded(ArtistSearchItem selectedItem)
    {
        if(recommendationFilters.get("seed_artists")!=null && recommendationFilters.get("seed_artists").containsKey(selectedItem.getArtistName()))
            return true;
        return false;
    }
